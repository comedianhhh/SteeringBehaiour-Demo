    ...
    public override Vector3 CalculateForce()
    {
        ...
        foreach(Feeler feeler in feelers)
        {
            ...
            if(Physics.Raycast(ray,out hit,feeler.distance,LayerMask.value))
            {
                ...
                float muliplier=1.0f+((feeler.distance-forceDirection.magnitude)/feeler.distance);

                Vector3 forcePosition=forceDirection+transform.position;
                forceDirection = (forcePosition - hit.point).normalized * muliplier * (1.0f / avoidWeight);
